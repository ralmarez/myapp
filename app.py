...
if "shuffled_deck" not in st.session_state:
    df = pd.read_csv(r"C:\Users\ralmarez\Downloads\flash.csv")
    st.session_state["shuffled_deck"] = df.sample(frac=1).reset_index(drop=True)
...
if not filtered_df.empty:
    ...
    st.warning("No cards match your filter. Try different Tense or Person values.")
